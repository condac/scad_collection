...
p3_y = 180;
p3_z = 2.5;
p3_zw = 17; // must be at least 7.5 // if you don't want wall change p3_wall to 0
p3_wall = 2; // side wall thickness
p3_digg = 1; // How much the components are lovered into the plates
...
arm_mount_y = 20;
 ...
 translate([0,0,+p3_z+under_spacer]) p1();
...
module p3() {
    ...
    difference() {
        ...
        translate([-p3_x/2+p3_wall, -p3_y/2+p3_wall, p3_z]) roundedcube(p3_x-p3_wall*2, p3_y-p3_wall*2, p3_z+p3_zw, 5);
            ...
            hull() {  
                translate([back_mount_x, back_mount_y, 0]) rotate([0,0,back_mount_angle])arm_mount(arm_w);
                translate([-back_mount_x, back_mount_y, 0]) rotate([0,0,180-back_mount_angle])arm_mount(arm_w);
            }
        }
        // cut to make sidewalls
        //translate([-p3_x/2+p3_wall, -p3_y/2+p3_wall, p3_z]) roundedcube(p3_x-p3_wall*2, p3_y-p3_wall*2, p3_z+p3_zw, 5);
...
module arm_mount_cut(in_z) {
    screw_d = 8;
    off = 4;
    translate([off,0,-1]) cylinder(d=3, h=p3_z+p3_zw, $fn=16);
    translate([off-screw_d,0,-1]) cylinder(d=3, h=p3_z+p3_zw, $fn=16);
}
...
module power_board() {
    ...
    translate([0, 43, 10]) union() {
        ...
        translate([0, 0, -50]) mount_holes(x=30);
    }
...
module video_tx() {
    ...
    v_z = 12;
    v_kontakt = 7;
    translate([0,p3_y/2-v_y/2-p3_wall+p3_wall/3,0]) union() {
        translate([-v_x/2, -v_y/2, 0]) cube([v_x, v_y, v_z]);
        translate([-v_kontakt/2, v_y/2, 0]) cube([v_kontakt, 11, v_kontakt+2]);
    }
}

module mount_holes(x=30) {
    dd=3;
    translate([x/2, x/2, 0]) cylinder(d=3, h=100);
    translate([-x/2, x/2, 0]) cylinder(d=3, h=100);
    translate([-x/2, -x/2, 0]) cylinder(d=3, h=100);
    translate([x/2, -x/2, 0]) cylinder(d=3, h=100);
}