...
p3_y = 180;
p3_z = 1.5;
p3_zw = 20; // must be at least 7.5 // if you don't want wall change p3_wall to 0
p3_wall = 2; // side wall thickness
p3_digg = p3_z/2; // How much the components are lovered into the plates
...
module p3_cut() {
    ...
    camera();
    translate([0,-6-6,p3_z]) cube([100, 18, 8]);
...
module mount_holes(x=30) {
    dd=3;
    translate([x/2, x/2, 0]) cylinder(d=screw_d, h=100);
    translate([-x/2, x/2, 0]) cylinder(d=screw_d, h=100);
    translate([-x/2, -x/2, 0]) cylinder(d=screw_d, h=100);
    translate([x/2, -x/2, 0]) cylinder(d=screw_d, h=100);
}