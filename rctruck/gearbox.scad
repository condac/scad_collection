...
translate([0,-60,output_h+46/2+14/2]) rotate([-90,0,0]) %motor();
module gear_holder() {
    ...
    difference() {
        union() {
            ...
            translate([0,0,0]) rotate([0,0,0]) cylinder(d=shaft, h=20); 
            translate([0,0,0]) rotate([0,0,0]) cylinder(d=shaft, h=30, $fn=4);
        }
...
module motormount() {
    ...
    difference() {
        union() {
            hull() {
                cylinder(d=20, h=4);
                translate([0,46/2+14/2,0]) cylinder(d=35, h=5);
...
translate([0,0,output_h]) rotate([90,0,0]) motormount();
module motor() {
    union() {
